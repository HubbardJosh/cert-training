...
Optional top-level parameters include \`system\` (a system prompt string applied before the conversation), \`temperature\` (controls output randomness, 0–1), \`top_p\`, \`top_k\`, and \`stop_sequences\` (strings that cause Claude to stop generating when encountered). These parameters tune Claude's behavior for specific use cases.`,
      quiz: [
        {
          question:
