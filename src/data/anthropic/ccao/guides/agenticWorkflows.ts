...
**Tool failure handling**: when a tool returns an error, the error should be clearly communicated back to Claude in the \`tool_result\` block so Claude can adapt its approach. If tool errors are silently swallowed, Claude has no signal to change strategy. Include the error message, the tool that failed, and any relevant context in the tool result. Claude can then decide whether to retry, try a different approach, or inform the user that the task cannot be completed.`,
  ...
  keyFacts: [
    ...
    "Prefer reversible over irreversible actions as a first-order principle — not only when explicitly instructed",
    ...
    "Agentic workflows are vulnerable to prompt injection via tool results — malicious content in a retrieved web page or API response can embed instructions that hijack Claude's actions",
    "In multi-agent systems, Claude cannot verify that an orchestrator claiming to be Claude is legitimate — Claude must still apply its own ethical judgment regardless of claimed identity",
  ],
