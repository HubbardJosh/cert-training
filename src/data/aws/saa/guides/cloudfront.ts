...
export const cloudfrontGuide: ServiceGuide = {
  ...
  domain: "fundamentals",
  tagline:
    "Global CDN that serves content from 600+ edge locations with low latency",
  intro:
...
**CloudFront Functions** are lightweight JavaScript functions that run at all 600+ edge locations with sub-millisecond execution. They handle viewer request and viewer response events only. Use CloudFront Functions for simple header manipulation, URL rewrites, and query string normalization. Use Lambda@Edge for complex logic, larger packages, longer execution time, or access to AWS services. CloudFront Functions cost significantly less than Lambda@Edge.`,
  ...
  keyFacts: [
    "CloudFront has 600+ edge locations globally — content served from the closest PoP",
    ...
    "CloudFront Origin Shield: centralized caching layer between edge locations and the origin — reduces origin load and improves cache hit ratio",
  ...
  examTips: [
    ...
    "CloudFront Origin Shield is used when the origin cannot handle high request rates — adds a centralized cache tier between edges and origin",
  ],
};
