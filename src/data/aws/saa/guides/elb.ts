...
ALB natively integrates with AWS WAF for Layer 7 protection, AWS Cognito for authentication, and ACM for TLS termination. ALB can route to EC2 instances, ECS containers (using dynamic port mapping), Lambda functions, and IP addresses. For containers, ECS registers targets with the ALB automatically using task metadata.`,
    ...
    },
    {
      ...
      body: `NLB operates at Layer 4 (TCP/UDP/TLS) and is designed for extreme performance — it handles millions of requests per second with significantly lower latency than ALB. NLB preserves the client's source IP address (ALB replaces it with its own IP). NLB assigns static IP addresses per AZ (or Elastic IPs), making it suitable for applications that whitelist specific IP addresses.
      ...
      quiz: [
        {
          ...
          correctIndex: 0,
          explanation:
            "NLB is the correct choice for non-HTTP TCP traffic requiring ultra-low latency. NLB also preserves the client source IP, making it visible to backend instances. ALB is HTTP/HTTPS only and replaces the source IP with its own. NLB is designed for ultra-low latency and significantly outperforms ALB for connection-based throughput.",
        },
      ],
    },
    {
...
For end-to-end encryption (not just termination), configure the target group to use HTTPS and ensure instances have their own certificates. For pass-through encryption where the load balancer does not decrypt, use NLB with TCP listeners — the TLS is terminated at the instance level, and the load balancer cannot inspect the payload.`,
  ...
  keyFacts: [
    ...
    "NLB supports TCP, UDP, and TLS — ALB supports HTTP, HTTPS, and WebSocket only",
  ],
  ...
  examTips: [
    ...
    "For gRPC-based microservice communication: use ALB with gRPC protocol version configured on the target group",
  ],
};
