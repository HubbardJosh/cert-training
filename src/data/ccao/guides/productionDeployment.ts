      ...
      quiz: [
        {
          ...
          correctIndex: 0,
          explanation:
            "Routing to Haiku where quality allows is the most impactful change when Haiku is 20-50x cheaper than Opus. 70% of requests moving to Haiku would dramatically cut the bill. Streaming does not affect token usage or cost. Reducing max_tokens by half saves output tokens on long responses but has no impact when responses are short. Prompt caching is valuable but produces at most ~10-20% savings on input tokens for a large system prompt — not comparable to a model switch for 70% of volume.",
        },
      ],
    },
    {
...
Tools in the LLM observability ecosystem include Anthropic's own usage dashboard in the Console (aggregate API usage), and third-party platforms like LangSmith, Helicone, Arize, and others that provide prompt tracing, evaluation tracking, and production monitoring. At minimum, every production Claude integration should log the request ID, model used, input/output token counts, latency, and any errors — even if using only basic logging infrastructure.`,
  ...
  keyFacts: [
    ...
    "Haiku is 20-50x cheaper than Opus at list pricing — model selection is the biggest cost lever",
  ...
  examTips: [
    ...
    "Model selection is the biggest cost lever — Haiku vs Opus is a 20-50x price difference",
