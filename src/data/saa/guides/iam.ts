...
When both an identity-based and a resource-based policy apply to a request, AWS uses a logical OR — access is allowed if either policy permits it (within the same account). An explicit **Deny** in any policy always overrides any Allow. **Permissions boundaries** set the maximum permissions an identity-based policy can grant to a user or role — they do not grant permissions themselves.`,
      quiz: [
        {
          question:
