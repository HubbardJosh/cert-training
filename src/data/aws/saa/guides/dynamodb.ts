...
**TTL** (Time to Live) automatically deletes items from a DynamoDB table when a specified timestamp attribute value passes. TTL deletions are free (no WCU consumed), processed within 48 hours of the expiry time. Use TTL for session tokens, temporary data, event logs older than a retention period, and any time-bounded data. Expired items are visible in queries until they are physically deleted — filter them out using a condition expression or FilterExpression if needed.`,
  ...
  keyFacts: [
    ...
    "PITR (Point-in-Time Recovery): continuously backs up the table, allowing restore to any second within the last 35 days",
    "Query: efficiently reads items with a specified partition key. Scan: reads the entire table (expensive at scale — avoid)",
  ],
  ...
  examTips: [
    ...
    "Enable PITR for production DynamoDB tables — 35 days of continuous backup with no performance impact",
    "Use Query instead of Scan — Scan reads every item in the table and filters in memory, consuming RCUs for all items read",
  ],
};
