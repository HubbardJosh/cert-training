...
**Redis vs. Memcached**: Redis supports persistence (RDB snapshots, AOF logging), replication, pub/sub, sorted sets, and geospatial indexing. Redis Cluster supports horizontal sharding across multiple shards. Memcached is simpler, multi-threaded, and supports no persistence — suitable for simple key-value caching where durability is not needed. For the exam: Redis is almost always the better choice for anything requiring persistence, replication, or complex data structures.`,
  ...
  keyFacts: [
    ...
    "RDS encryption must be enabled at creation time — to encrypt an existing unencrypted instance: snapshot → copy snapshot with encryption enabled → restore from encrypted snapshot",
    "RDS IAM database authentication: connect using a short-lived IAM token instead of a password — supported for MySQL and PostgreSQL",
  ],
  ...
  examTips: [
    ...
    "To migrate an unencrypted RDS instance to encrypted: snapshot → copy with encryption → restore (cannot encrypt in-place)",
    "RDS storage auto-scaling automatically increases storage capacity when running low — this is storage scaling, not read scaling (which uses read replicas)",
  ],
};
